...
// Função para inserir os itens na tabela, com checkbox individual
function insertItemCancelado(item, index) {
    ...
    // Cria a linha da tabela com o checkbox da linha
    tr.innerHTML = `
      <td><input type="checkbox" class="row-checkbox" onclick="checkIndividualCheckboxes()"></td> <!-- Checkbox individual -->
    ...
    document.getElementById('tbodyCancelado').appendChild(tr); // Insere o <tr> no tbody
}

// Função para selecionar/desselecionar todos os checkboxes
function toggleSelectAll() {
    const selectAllCheckbox = document.getElementById('selectAll'); // Checkbox "Selecionar Todos"
    const checkboxes = document.querySelectorAll('.row-checkbox');  // Checkboxes individuais nas linhas

    checkboxes.forEach(checkbox => {
        checkbox.checked = selectAllCheckbox.checked; // Marca/desmarca conforme o checkbox mestre
    });
}

// Função para verificar se todos os checkboxes individuais estão marcados
function checkIndividualCheckboxes() {
    const selectAllCheckbox = document.getElementById('selectAll'); // Checkbox "Selecionar Todos"
    const checkboxes = document.querySelectorAll('.row-checkbox');  // Checkboxes individuais

    const allChecked = Array.from(checkboxes).every(checkbox => checkbox.checked); // Verifica se todos estão marcados
    selectAllCheckbox.checked = allChecked; // Atualiza o checkbox principal
}
